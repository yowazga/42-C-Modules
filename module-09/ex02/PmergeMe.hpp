...
Vit ranged_binary_search(Vit begin, Vit end, int value, size_t unit_size)
{
	...
	while (begin < end)
	{// [[  37
		size_t count_units = std::distance(begin, end) / unit_size; // 1
		Vit mid_it = begin + ((count_units / 2) * unit_size);
...
void pant_to_mainchchain(Vec& pent, Vec& mainchain, Vec& mainchain_idxs, size_t unit_size, size_t unit)
{
	typedef typename Vec::iterator Vit;
	Vit it = pent.begin() + (unit_size * unit);
	Vit it_end = it + unit_size - 1;
	Vit mchain_end_toCheck = mainchain.begin() + mainchain_idxs[unit];
	Vit insertion_pos = ranged_binary_search(mainchain.begin(), mchain_end_toCheck, *it_end, unit_size);
	size_t insertion_idx = std::distance(mainchain.begin(), insertion_pos);
	mainchain.insert(insertion_pos, it, it_end + 1);
	mainchain_idxs.erase(mainchain_idxs.begin() + unit); // &mainchain_idxs[unit]
	for (Vit it_mch_index = mainchain_idxs .begin(); it_mch_index != mainchain_idxs.end(); it_mch_index++)
		if ((size_t)*it_mch_index >= insertion_idx)
			(*it_mch_index) += unit_size;
